...
def call_model(prompt, options=None):
    payload = {'model':'meta-llama/Meta-Llama-3.1-70B-Instruct','messages':[{'role':'user','content': prompt + (f" Options: {'; '.join(options)}" if options else '')}]}
    r = requests.post(ENDPT, headers={'Authorization':f"Bearer {API_KEY}",'Content-Type':'application/json'}, json=payload)
    return r.json()['choices'][0]['message']['content'].strip()
# EA
oea='results_meta_llama_ea.csv'
...
print(f"Meta-LLaMA EA {ea:.3f}")

# EU
eu_fn='results_meta_llama_eu.csv'
...
for cat in set(x['Category'] for x in data_eu):
    ...
    print(f"Meta-LLaMA EU {cat}:{eu:.3f}")